...
function checkFrontMatter(filePath) {
  try {
    const fileContents2 = (0, import_fs.readFileSync)(filePath, "utf8");
    const parsed = (0, import_front_matter.default)(fileContents2, { allowUnsafe: true });
    load(parsed.attributes);
    console.log(`\u2705 ${filePath}` + import_chalk.default.green(` passed.`));
    return true;
  } catch (e) {
    hasErrors = true;
    const errorMessage = import_chalk.default.red(`\u274C Invalid front matter in ${filePath}: ${e.message}`);
    ...
    if (e instanceof YAMLException && e.mark) {
      ...
      console.error(import_chalk.default.yellow(`   at line ${line + 1}, column ${column + 1}`));
      console.error(import_chalk.default.yellow(`   Context: "${context}"`));
    }
...
function walkDirectory(directory) {
  ...
  for (const file of files) {
    const fullPath = (0, import_path.join)(directory, file);
    const stat = (0, import_fs.statSync)(fullPath);
    if (stat.isDirectory()) {
      walkDirectory(fullPath);
    } else if ((0, import_path.extname)(fullPath) === ".md") {
...
if (hasErrors) {
  ...
  errorSummary.forEach((error, index) => {
    console.log(import_chalk.default.red(`
Error ${index + 1}:`));
    console.log(import_chalk.default.red(`File: ${error.filePath}`));
    console.log(import_chalk.default.red(`Reason: ${error.message}`));
    if (error.context) {
      console.log(import_chalk.default.yellow(`${error.context}`));
    }
  });
  process.exitCode = 1;
} else {
  console.log(import_chalk.default.green("All front matter passed."));
}
